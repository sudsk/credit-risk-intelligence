...
In production, this would connect to Core Banking System or Open Banking APIs.
"""
...
async def list_tools() -> list[Tool]:
    """List available financial data tools."""
    return [
        Tool(
            name="get_financial_metrics",
            description="Get current quarter financial metrics and ratios",
            inputSchema={
                "type": "object",
                "properties": {
                    "sme_id": {
        ...
        Tool(
            ...
            description="Get quarterly revenue trend analysis",
            inputSchema={
                "type": "object",
                "properties": {
                    "sme_id": {
        ...
        Tool(
            ...
            description="Analyze liquidity position (cash runway, current ratio, quick ratio)",
            inputSchema={
                "type": "object",
                "properties": {
                    "sme_id": {
        ...
        Tool(
            ...
            description="Analyze profitability metrics (margins, ROA, ROE)",
            inputSchema={
                "type": "object",
                "properties": {
                    "sme_id": {
        ...
        Tool(
            name="get_leverage_analysis",
            description="Analyze debt levels and coverage ratios",
            inputSchema={
                "type": "object",
                "properties": {
                    "sme_id": {
...
async def call_tool(name: str, arguments: dict) -> list[TextContent]:
    ...
    if name == "get_financial_metrics":
        ...
        if fin_row.empty:
            return [TextContent(
                type="text",
                text=f"No financial data found for SME {sme_id}"
            )]
        ...
        return [TextContent(
            type="text",
            text=f"Financial Metrics (Latest Quarter):\n{pd.Series(metrics).to_string()}"
        )]
        ...
        if fin_row.empty:
            return [TextContent(
                type="text",
                text=f"No revenue data found for SME {sme_id}"
            )]
        ...
        if fin_row.empty:
            return [TextContent(
                type="text",
                text=f"No liquidity data found for SME {sme_id}"
            )]
        ...
        return [TextContent(
            type="text",
            text=f"Liquidity Analysis:\n{pd.Series(liquidity).to_string()}"
        )]
    
    elif name == "get_profitability_analysis":
        fin_row = financial_df[financial_df['sme_id'] == sme_id]
        
        if fin_row.empty:
            return [TextContent(
                type="text",
                text=f"No profitability data found for SME {sme_id}"
            )]
        ...
        return [TextContent(
            type="text",
            text=f"Profitability Analysis:\n{pd.Series(profitability).to_string()}"
        )]
    
    elif name == "get_leverage_analysis":
        fin_row = financial_df[financial_df['sme_id'] == sme_id]
        
        if fin_row.empty:
            return [TextContent(
                type="text",
                text=f"No leverage data found for SME {sme_id}"
            )]
        ...
        return [TextContent(
            type="text",
            text=f"Leverage Analysis:\n{pd.Series(leverage).to_string()}"
        )]
    
    elif name == "assess_financial_health":
        fin_row = financial_df[financial_df['sme_id'] == sme_id]
        ...
        return [TextContent(
            type="text",
            text=f"Financial Health Assessment:\n{pd.Series(assessment).to_string()}\n\nKey Concerns:\n{concerns_text}"
        )]
...
def _identify_financial_concerns(data: pd.Series) -> list[str]:
